...
export default function CollectionsPage() {
  ...
  return <main className="min-w-0 flex-1 overflow-y-auto bg-white px-6 py-8 text-[#171717] md:px-10">
    <div className="mx-auto max-w-[1180px]">
      ...
      <div className="mt-5 overflow-hidden rounded-xl border border-black/10"><div className="grid grid-cols-[1fr_120px_120px] bg-black/[.025] px-4 py-2.5 text-[10px] text-black/40"><span>Name</span><span>Fields</span><span>Updated</span></div>{visible.map((collection) => <Link href={`collections/${collection._id}`} key={collection._id} className="grid grid-cols-[1fr_120px_120px] items-center border-t border-black/10 px-4 py-4 hover:bg-black/[.018]"><div className="flex items-center gap-3"><span className="grid size-9 place-items-center rounded-lg bg-black text-white"><Database className="size-4"/></span><div><h2 className="text-sm font-medium">{collection.name}</h2><p className="text-[11px] text-black/40">{collection.slug}</p></div></div><span className="text-xs text-black/50">{collection.fields.length}</span><span className="text-xs text-black/50">{new Intl.DateTimeFormat(undefined,{dateStyle:"medium"}).format(collection.updatedAt)}</span></Link>)}{collections !== undefined && visible.length === 0 ? <div className="py-16 text-center text-sm text-black/40">No collections found.</div> : null}</div>
    </div>
